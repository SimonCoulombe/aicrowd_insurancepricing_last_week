...
create__group_folds <- function(df, group_var = id_policy, n_folds = 5, seed = 42) {
  set.seed(seed)
  group_var_values <-  df %>% distinct( {{group_var}}) # everyone has 4 records, always put someone's records in the same fold
...
fit_model <- function(x_raw= NULL, 
  ...
  final_model_oof_preds <- training %>% select(id_policy, year) %>% bind_cols(tibble(preds = xgcv$pred))
